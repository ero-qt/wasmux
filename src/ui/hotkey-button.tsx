 ...
 * injected into a visually-hidden `<span>` so the accessible name is derived
 * from the button's contents (visible text + hidden combo), satisfying
 * WCAG 2.5.3 Label in Name without an overriding `aria-label`.
...
export const HotkeyButton: Component<HotkeyButtonProps> = (props) => {
  ...
  return (
    ...
    >
      {props.children}
      <Show when={combos()}>{(c) => <span class={visuallyHidden}> ({c()})</span>}</Show>
    </button>
  );
};
