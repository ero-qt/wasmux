...
describe("RationalTime", () => {
  ...
  test("creates a reduced rational time", () => {
    const t = createTime(6, 24);
    expect(t.num).toBe(1);
    expect(t.den).toBe(4);
  });

  test("preserves sign in the numerator", () => {
    const t = createTime(-3, 24);
    expect(t.num).toBe(-1);
    expect(t.den).toBe(8);
  });
  ...
  test("represents zero with denominator 1", () => {
    const t = createTime(0, 24);
    expect(t.num).toBe(0);
    expect(t.den).toBe(1);
  });

  test("throws on zero denominator", () => {
    expect(() => createTime(1, 0)).toThrow();
  });

  test("throws on non-integer numerator", () => {
    expect(() => createTime(1.5, 24)).toThrow();
  });

  test("throws on non-integer denominator", () => {
    expect(() => createTime(1, 23.976)).toThrow();
  });
  ...
  test("compares equal times as 0", () => {
    expect(compare(createTime(1, 24), createTime(1, 24))).toBe(0);
  });

  test("compares equal times with different denominators as 0", () => {
    // 3/24 = 5/40
    expect(compare(createTime(3, 24), createTime(5, 40))).toBe(0);
  });

  test("compares less-than as -1", () => {
    expect(compare(createTime(1, 24), createTime(2, 24))).toBe(-1);
  });

  test("compares greater-than as 1", () => {
    expect(compare(createTime(2, 24), createTime(1, 24))).toBe(1);
  });

  test("eq returns true for equal times across rates", () => {
    expect(eq(createTime(1, 2), createTime(12, 24))).toBe(true);
  });

  test("lt, lte, gt, gte work correctly", () => {
    const a = createTime(1, 24);
    const b = createTime(2, 24);
  ...
  test("rescales to a new denominator without loss", () => {
    // 1/24 = 5/120
    const result = rescale(createTime(1, 24), 120);
    expect(result.num).toBe(5);
  ...
  test("rescale floors by default when conversion is not exact", () => {
    ...
    const result = rescale(createTime(1, 24), 30);
    expect(result.num).toBe(1);
    expect(result.den).toBe(30);
  });

  test("rescale ceils when requested", () => {
    const result = rescale(createTime(1, 24), 30, "ceil");
  ...
  test("rescale rounds when requested", () => {
    ...
    const a = rescale(createTime(5, 24), 30, "round");
    expect(a.num).toBe(6);
    expect(a.den).toBe(30);

    // 7/24 at 30fps: 7/24 * 30 = 8.75. round = 9.
    const b = rescale(createTime(7, 24), 30, "round");
    expect(b.num).toBe(9);
  ...
  test("rescale preserves exact values", () => {
    ...
    const result = rescale(createTime(1, 4), 24);
  ...
  test("isZero returns true for zero", () => {
    ...
    expect(isZero(createTime(0, 48000))).toBe(true);
  });

  test("isZero returns false for non-zero", () => {
    expect(isZero(createTime(1, 24))).toBe(false);
  });
  ...
  test("concatenating clip durations does not accumulate error", () => {
    ...
    const frame = createTime(1001, 30000);
  ...
  test("round-trip through rate conversion preserves value", () => {
    const original = createTime(5, 24);
  ...
  test("NTSC 29.97fps frame is representable exactly", () => {
    ...
    const oneFrame = createTime(1001, 30000);
  ...
  test("handles common video rates without overflow", () => {
    ...
    for (const rate of rates) {
      ...
      const oneHour = createTime(3600 * rate, rate);
