...
- app shell: CSS-grid layout with named regions (header / bin / program / inspector / timeline / status), each non-header region a semantic landmark with `aria-label`. Placeholder text fills regions that have no content yet; floating panels for perf and jobs are deferred to a later commit
...
- floating panel system: module-level open-state store (`isPanelOpen` / `openPanel` / `closePanel` / `togglePanel`), `<FloatingPanel>` overlay component (open/close, fixed default position via `top-right`/`bottom-right`/`bottom-left` presets, no drag/resize), `<PanelOverlay>` container that pipes pointer events through empty space
...
- `j` toggles the jobs panel; `g` fires a synthetic two-child demo job (dev only)
