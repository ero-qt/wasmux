...
- PWA: `public/manifest.webmanifest` with SVG icons (the existing favicon and a dedicated maskable variant with safe-zone padding), `<link rel="manifest">` + `apple-touch-icon` + `theme-color` in `index.html`; hand-rolled service worker at `public/sw.js` (precache app shell, cache-first runtime, version-bumped cache cleanup); registration gated to production builds in `main.tsx`
- app shell: CSS-grid layout with named regions (header / bin / program / inspector / timeline / status), each content region a semantic landmark with `aria-label`
- theme store at `src/ui/theme/theme-store.ts`: initial value from `prefers-color-scheme`, applied to `<html>`'s class via a Solid effect; `toggleTheme` / `setActiveTheme` mutators
- header `ThemeToggle` button bound to `t`, label names the *target* theme, `aria-pressed` reflects the *current* theme
- jobs UI: singleton `jobTracker` plus reactive `reactiveRoots` / `reactiveChildren` / `reactiveChain` accessors backed by a single bump signal; collapsible tree (oldest at top, expand/collapse on nodes with children) with status icons; `<JobChainWidget>` in the status bar showing the deepest running leaf as `Render › Encode › Frame 42%`
- behavioral component tests added via `@solidjs/testing-library`
- workspace docking: each content zone (bin / program / inspector / timeline) is toggleable via the Kobalte-powered `<ViewsMenu>` in the header, matching Premiere / Resolve conventions. Hidden zones collapse out of the grid. The jobs panel docks into the inspector zone; floating / drag-out is deferred until there's a real second monitor use case
- `g` fires a synthetic two-child demo job (dev only)
